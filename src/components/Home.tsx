          ...
          {newsList.length > 1 ? (
            newsList.slice(1).map((news) => (
              <div
                ...
                className="bg-white p-4 rounded-xl shadow-sm hover:shadow-xl transition-all flex gap-4 group cursor-pointer border border-gray-100"
              >
                <div className="w-1/3 aspect-video shrink-0 overflow-hidden rounded-lg">
                  <img 
                    src={news.imageUrl} 
                    alt={news.title} 
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[10px] bg-sami-red/10 text-sami-red px-2 py-0.5 rounded font-black uppercase tracking-wider">{news.category}</span>
                  </div>
                  <h3 className="font-bold text-sm leading-snug group-hover:text-sami-red transition-colors line-clamp-2">
                    {news.title}
                  </h3>
                  <div className="flex items-center gap-2 text-[10px] text-gray-400 mt-3">
                    <Clock size={10} />
            ...
            [1, 2, 3, 4].map((i) => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-400 mt-2">২ ঘণ্টা আগে</p>
                </div>
              </div>
            ))
          )}
          ...
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-3">
                <div className="w-2 h-8 bg-sami-red rounded-full"></div>
                <h2 className="text-2xl font-black text-gray-900 tracking-tight">আমাদের পরিবার</h2>
              </div>
              <button 
                onClick={() => onNavigate('/family')}
                className="bg-sami-red/10 text-sami-red px-4 py-2 rounded-full text-xs font-black hover:bg-sami-red hover:text-white transition-all uppercase tracking-widest"
              >
                সব দেখুন
              </button>
            </div>
              ...
              {reporters.map((rep) => (
                <div key={rep.id} className="flex flex-col items-center text-center group cursor-pointer" onClick={() => onNavigate('/family')}>
                  <div className="relative w-20 h-20 mb-3">
                    <div className="absolute inset-0 bg-sami-red rounded-full scale-0 group-hover:scale-105 transition-transform duration-300 opacity-20"></div>
                    <div className="w-full h-full rounded-full overflow-hidden relative z-10 border-2 border-white shadow-md group-hover:border-sami-red transition-all">
                      <img src={rep.imageUrl} alt={rep.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    </div>
                  </div>
                  <h3 className="text-xs font-black text-gray-900 line-clamp-1 group-hover:text-sami-red transition-colors">{rep.name}</h3>
                  <p className="text-[10px] text-gray-400 font-bold line-clamp-1">{rep.designation}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {mediaItems.length > 0 && (
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-3">
                <div className="w-2 h-8 bg-sami-red rounded-full"></div>
                <h2 className="text-2xl font-black text-gray-900 tracking-tight">মিডিয়া গ্যালারি</h2>
              </div>
              <button 
